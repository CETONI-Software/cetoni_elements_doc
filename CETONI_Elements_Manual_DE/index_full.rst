CETONI Elements Handbuch
=========================================

.. image:: _static/splashscreen_dark.jpg

**Willkommen beim CETONI Elements Handbuch!**

Die CETONI Elements Software ist eine modulare Laborautomatisierungslösung zur
Steuerung und Automatisierung aller CETONI-Geräte sowie zur Integration und
Automatisierung externer Geräte.

Verwendete Symbole und Signalwörter
------------------------------------

Die folgenden Symbole werden in diesem Handbuch verwendet und sollen Sie bei der
Navigation durch dieses Dokument unterstützen:

.. admonition:: Tipp
   :class: tip

   Bezeichnet Anwendungstipps und nützliche Hinweise die den Umgang mit der
   Software erleichtern

.. admonition:: Wichtig
   :class: note

   Bezeichnet wichtige Hinweise und andere besonders nützliche Informationen,
   wobei keine gefährlichen oder schädlichen Situationen auftreten.

.. admonition:: Achtung
   :class: caution

   Bezeichnet eine möglicherweise schädliche Situation. Wenn sie nicht gemieden
   wird, kann das Produkt oder etwas in seiner Umgebung beschädigt werden

.. admonition:: Vorsicht
   :class: error

   Bezeichnet eine möglicherweise gefährliche Situation. Wenn sie nicht gemieden
   wird, können leichte oder geringfügige Verletzungen sowie Sachschäden die
   Folge sein. der Gerätetreiber leicht variieren.


.. toctree::
   :maxdepth: 2
   :caption: Inhaltsverzeichnis:
   :hidden:
   :numbered: 3

   Installation_DE/Installation_DE.rst
   QmixElementsADS_DE/QmixElementsADS_DE.rst
   qmixdevices_DE/qmixdevices_full_DE.rst
   scriptingsystem_part1_DE/scriptingsystem_part1_DE.rst
   scriptingsystem_part2_DE/scriptingsystem_part2_DE.rst
   javascript_DE/javascript_DE.rst
   nemesys_DE/nemesys_full_DE.rst
   qmixio_DE/qmixio_full_DE.rst
   qmixcontroller_DE/qmixcontroller_full_DE.rst
   qmixv_DE/qmixv_full_DE.rst
   tubingpump_DE/tubingpump_DE.rst
   rotaxys_DE/rotaxys_DE.rst
   balance_DE/balance_DE.rst
   ledarray_DE/ledarray_DE.rst
   cameraview_DE/cameraview_DE.rst
   imageanalysis_DE/imageanalysis_DE.rst
   videobuilder_DE/videobuilder_DE.rst
   datalogger_DE/datalogger_full_DE.rst
   graphlogger_DE/graphlogger_full_DE.rst
   database_DE/database_DE.rst
   sila_DE/sila_DE.rst
   spectroscopy_DE/spectroscopy_DE.rst
   daq_DE/daq_DE.rst
   canopentools_DE/canopentools_DE.rst
   Appendix_DE/Appendix_DE.rst
